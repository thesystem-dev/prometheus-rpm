...
| `verify` | Built RPMs under `runtime/artifacts/` plus the staged signing material required by `scripts/sign-rpms.sh --verify`; see [signing.md](signing.md) |
...
| `verify` | `--rpm-types <binary\|source\|both>` (default: `both`), `--input-dir <dir>` (default: `runtime/artifacts`), `--runtime <dir>` (default: `runtime`), `-h`, `--help` |
